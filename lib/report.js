...
async function getReportList(reportPath, forceRefresh) {
    if (!isCacheable) {
        ...
        return await reportExecution.listChildren(reportPath)
    }
}
...
async function cacheReportList(reportPath, keepHidden) {
    ...
    for (var i = 0; i < reports.length; i++) {
        ...
        if (reports[i].TypeName === "Folder") {
            ...
            reportList[reportPath] = [];
        } else if (reports[i].TypeName === "ReportItem") {
            if (keepHidden) {
                reportList[reportPath].push(reports[i]);
            } else {
...
async function download(reportPath) {
    ...
    while (reportPath.length) {
        ...
        for (var i = 0; i < result.length; i++) {
            var file = { name: result[i].Name, path: result[i].Path, type: result[i].TypeName, definition: definition };
