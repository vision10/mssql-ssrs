...
async function uploadFiles(filePath, reportPath, options) {
    ...
    if (options) {
        ...
        if (Array.isArray(inc.dataSources)) { files.dataSources = files.folders.concat(inc.dataSources) }
...
async function fixDataSourceReference(reportPath, dataSourcePath, logger) {
    ...
    var reports = items.filter(r => r.Type === 'Report');
    var ds = items.filter(r => r.Type === 'DataSource').map(r => {
        var res = {};
        res[r.Name] = r.Path;
        return res
    });
    ...
    var result = await setReferences(reports, ds, reportPath, log);
    return result;
}
