...
async function start(url, auth, security, useRs2012) {
    try {
        var cli = await soap.createClient(url + (useRs2012 ? asmx.rs2012 : asmx.rs2010), auth, security);
        reportService = cli;
        if (useRs2012) {
...
async function setProperties(reportPath, properties) {
    try {
        ...
        return await client.SetProperties({ ItemPath: reportPath, Properties: { Property: props } });
    } catch (err) { report.errorHandler(err) }
}
...
async function getItemReferences(itemPath, referenceItemType) {
    try {
        var result = await client.GetItemReferences({ ItemPath: itemPath, ReferenceItemType: referenceItemType });
...
async function setItemReferences(itemPath, itemReferences) {
    try {
        var result = await client.SetItemReferences({ ItemPath: itemPath, ItemReferences: { ItemReference: itemReferences } });
        return result;
    } catch (error) { report.errorHandler(error) }
}