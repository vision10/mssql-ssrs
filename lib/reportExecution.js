...
async function start(url, config, options, security) {
    try {
        if (!options) { options = {} }
        if (options.rootFolder) { soap.setRootFolder(options.rootFolder); }
...
function arrayToReport(params, checkNulls) {
    ...
    for (var i = 0; i < params.length; i++) {
        if (params[i].ParameterTypeName === "DateTime" || params[i].Value instanceof Date) {
            formated.push({
                Name: params[i].Name,
                Value: moment(params[i].Value).format("MM/DD/YYYY")
            });
...
async function getReportByUrlNew(reportPath, fileType, params, auth) {
    try {
        var config = {
            binary: true, // very important
            username: auth.userName,
            ...
            workstation: auth.workstation,
            domain: auth.domain,
            url: (testReportPath(reportPath).replace(/\s/g, '+'))
                + "&rs:Command=Render&rs:Format=" + reportFormat(fileType)
                + formatParamsToUrl(params)
        };
    } catch (err) { report.errorHandler(err) }
    ...
    if (Array.isArray(params)) {
        ...
        for (var key in params) {
            if (params[key] instanceof Date && !isNaN(params[key].valueOf())) {
                ...
                urlParts.push(key + (params[key] === null || params[key] === undefined ? ':IsNull=True' : "=" + params[key]));
            }
        }
    }
