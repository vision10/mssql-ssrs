...
module.exports = class ReportExecutionUrl {
    ...
    async getReport(reportPath, fileType, params, axiosConfig) {
        if (!this.client) {
            ...
            this.client = NtlmClient(credentials, Object.assign(defaults, this.axiosCfg));
        }
        const urlPath = utils.testReportPath(this.soapInstance.getRootFolder(), reportPath).replace(/\s/g, '+');
        const rsParams = `&rs:Command=Render&rs:Format=${utils.reportFileFormat(fileType)}`;
        const baseUrl = `${this.soapInstance.getServerUrl()}?${encodeURIComponent(urlPath)}${rsParams}`;
        const formBody = formatParamsToUrl(params);
        if (((axiosConfig && axiosConfig.method) || '').toLowerCase() === 'post') {
            return await this.client({
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                method: 'post', url: baseUrl, data: formBody, 
            });
        } else {
            return await this.client(`${baseUrl}&${formBody}`);
        }
    }
}
    ...
    if (Array.isArray(params)) { // [{Name: nume, Value: valoare}]
        ...
        for (var key in params) {
            ...
            if (value instanceof Date && !isNaN(value.valueOf())) {
                ...
                urlParts.push(key + "=" + parts.join('%2C'))
